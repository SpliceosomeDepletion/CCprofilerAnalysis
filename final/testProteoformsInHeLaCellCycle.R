...
if (length(grep("nas21.ethz.ch",getwd()))>0) {
  ...
  load_all()
  setwd("~/mysonas/PRPF8/analysis/output/HeLa_CellCycle")
  knitr::opts_knit$set(root.dir = '~/mysonas/PRPF8/analysis/output/HeLa_CellCycle')
} else {
  setwd("/Volumes/ibludau-1/CCprofiler")
  load_all()
  setwd("/Volumes/ibludau-1/PRPF8/analysis/output/HeLa_CellCycle")
  knitr::opts_knit$set(root.dir = '/Volumes/ibludau-1/PRPF8/analysis/output/HeLa_CellCycle')
}
...
pep_traces <- pep_traces[[2]]
pep_traces <- pep_traces[[5]]
...
for (test_protein in test_proteins){
  ...
  test_protein_name <- pepTest$trace_annotation$protein_id[1]
