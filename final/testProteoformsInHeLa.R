...
if (length(grep("nas21.ethz.ch",getwd()))>0) {
  ...
  load_all()
  setwd("~/mysonas/PRPF8/analysis/output/HeLa")
  knitr::opts_knit$set(root.dir = '~/mysonas/PRPF8/analysis/output/HeLa')
} else {
  setwd("/Volumes/ibludau-1/CCprofiler")
  load_all()
  setwd("/Volumes/ibludau-1/PRPF8/analysis/output/HeLa")
  knitr::opts_knit$set(root.dir = '/Volumes/ibludau-1/PRPF8/analysis/output/HeLa')
}

#' ## Load data
pep_traces <- readRDS("../../../../html/SEC_Hela_Peng/Spectronaut/raw_pepTraces.rds")
calibration <-  readRDS("../../../../html/SEC_Hela_Peng/Spectronaut/calibration.rds")

pep_traces <- pep_traces[[1]]
...
single_proteoform <- traces_combined_minCorr_pval_clustered$trace_annotation[proteoform_pval_adj > 0.1]
multi_proteoforms <- traces_combined_minCorr_pval_clustered$trace_annotation[proteoform_pval_adj <= 0.1]
