resolveProteoformSpecificFeatures <- function(features, traces, minProteoformIntensityRatio=0.1, perturb_cutoff="5%"){
  ...
  res <- lapply(idx, function(i){
    ...
    if (length(grep("DECOY",x$protein_id)) == 0) {
      ...
      if (length(proteoforms_detected) > 1) {
        ...
        #  return(NULL)
        #}
        intensityRatioPerProteoform <- lapply(maxPeptideSumPerProteoform, function(x){x/max(unlist(maxPeptideSumPerProteoform))})
